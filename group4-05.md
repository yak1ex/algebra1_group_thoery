...
※(2) シロー $2$ 部分群の場合には $s=1,3,9$ となり $s=1,3$ は同様の議論が使えるが $s=9$ のときには単射性を否定できないので同様の議論が使えない。(1) でもシロー $3$ 部分群を選ぶと同じ議論にはならないし (3) でもシロー $3$ 部分群を選ぶと同じ議論にはならない。
...
※バーンサイドの定理で $p^aq^b$ の形が可解群であることが言える。可解群であれば定義から自身がアーベル群であるか非自明な正規部分群が存在することが言える。(命題4.3.11でも定義より明らかと言っているが、p.115 命題4.3.11の対偶から可解群であれば $G$ は非可換単純群ではない)