...
(※) $\mathbb{Z}/n\mathbb{Z}$について自己同型写像の定義域は$\mathbb{Z}/n\mathbb{Z}$、つまり $\overline{1}$を生成元とする巡回群なのでその対応先だけ決めれば写像は決まる(ref p.44 命題2.5.12)。演習問題2.5.3より同型な写像の場合、位数が等しい元に写す必要がある。演習問題2.4.6の結果を使って元の位数が$\overline{1}$(位数は$n$)と等しい元を求めると$n$と素な$m$に対する$\overline{m}$となる。以下$\overline{1}$を$\overline{m}$に写す写像を$\phi_{\overline{m}}$と表記する(このとき$\phi_{\overline{m}}(\overline{x}) = \phi_{\overline{m}}(\overline{1}^x) = \phi_{\overline{m}}(\overline{1})^x = \overline{m}^x = \overline{1}^{mx} = \overline{x}^m$となり演習問題2.5.2よりwell-definedな準同型写像である)。上の議論より$\phi$が$\mathbb{Z}/n\mathbb{Z}$の自己同型であれば、$n,m$素な$\phi_{\overline{m}}$として表せることが言える。一方、$n,m$素な$\phi_{\overline{m}}$を考えると上の議論からこれはwell-definedな準同型写像なので、全単射であることを言えば$\phi_{\overline{m}}$が$\mathbb{Z}/n\mathbb{Z}$の自己同型であることが言え、$\{ \phi_{\overline{m}} \}$が$\mathbb{Z}/n\mathbb{Z}$の自己同型群と同型であることが言える。$\phi_{\overline{m}}$が全単射であることを示す。$\phi_{\overline{m}}(\overline{x}) = \phi_{\overline{m}}(\overline{y}) \Rightarrow \overline{1}^{mx} = \overline{1}^{my} \Rightarrow \overline{1}^{mx-my} = \overline{0} \Rightarrow \overline{1}^{m(x-y)} = \overline{0}$。p.39 命題2.4.18より$n \mid m(x-y)$(※$\overline{1}$の位数は$n$である)。$n,m$素のため、$n \mid x-y$。つまり$\overline{x} = \overline{y}$である。したがって$\phi_{\overline{m}}$は単射。$\mathbb{Z}/n\mathbb{Z}$の位数は有限なので p.7 命題1.1.6(2)より $\phi_{\overline{m}}$は全単射。したがって、$\{ \phi_{\overline{m}} \}$が$\mathbb{Z}/n\mathbb{Z}$の自己同型群と同型である。このとき$xy$を$n$で割った余りを$r$とする($xy = nq + r, q \in \mathbb{Z}, r \in \mathbb{Z}, 0 \le r \lt n$)と、$(\phi_{\overline{x}} \circ \phi_{\overline{y}}) (\overline{1}) = \phi_{\overline{x}} (\phi_{\overline{y}} (\overline{1})) = \phi_{\overline{x}}(\overline{y}) = \phi_{\overline{x}}(\overline{1}^y) = (\phi_{\overline{x}}(\overline{1}))^y = \overline{x}^y = \overline{1}^{xy} = \overline{r} = \phi_{\overline{r}}(\overline{1})$となるため、$(\phi_{\overline{x}} \circ \phi_{\overline{y}}) = \phi_{\overline{r}}$(p.44 命題2.5.12より生成元$\overline{1}$の写像先を定めれば写像全体が決まるため)。 これは$\mathbb{Z}/n\mathbb{Z}$の自己同型群$\{ \phi_{\overline{m}} \}$が$(\mathbb{Z}/n\mathbb{Z})^\times$と同型ということである。
