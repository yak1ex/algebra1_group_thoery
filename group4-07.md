...
p.98 命題4.1.10より、二面体群 $D_p$ は $t^p=1,r^2=1,rtr=t^{-1}$ を満たす $t,r$ で生成され、位数は $2p$ である。よって、p.124 定理4.6.5より $F=\langle x,y\mid x^p=1,y^2=1,yxy=x^{-1}\rangle$ から $D_p$ への全射準同型 $\phi$ が存在する。 $yxy=x^{-1}$ から $yx=x^{-1}y$ なので $x$ を左、 $y$ を右に集めることができ、また $x^p=1,y^2=1$ だから $F$ の元は全て $x^iy^j (i=0,\cdots,p-1,j=0,1)$ の形で表すことができる。よって $F$ の位数は高々 $2p $である。 $F$ から $D_p$ への全射があるので $2p\ge|F|\ge|D_p|=2p$ となり、 $|F|=2p$ である。 $|G|=2p$ なので $F$ から $G$ の全射準同型 $\phi$ は全単射準同型になるので $F\cong D_p$ となる。
...
したがって、$n$ は $2,\cdots,p-1$ のいずれかである。
両側から $a$ をかけると $b=ab^na$ 、これを代入して $aba=b^n=(ab^na)^n=ab^{n^2}a$ となるから両側から $a$ をかけて $b=b^{n^2}$ となる。さらに $b^{-1}$ をかけると、 $1=b^{n^2-1}$ となるからp.43 命題2.4.19より $n^2-1=(n-1)(n+1)$ が $p$ の倍数となる。 $p$ は素数だから $n-1,n+1$ のいずれかが $p$ の倍数である。 $n$ は $2,\cdots,p-1$ のいずれかで $p$ が素数であることから $n-1$ は $p$ の倍数にならない。また $n+1$ が $p$ の倍数になるのは $n+1=p$ の場合のみである。つまり $n=p-1$ となる。よって $aba=b^{n-1}=b^{-1}$ である。

以上から、 $G=\langle a,b\rangle, b^p=1, a^2=1, aba=b^{-1}$ となるので、p.124 定理4.6.5より $F=\langle x,y\mid x^p=1,y^2=1,yxy=x^{-1}\rangle$ から $G$ への全射準同型 $\phi$ が存在する。 $D_p$ に対する議論と同様の議論で $F\cong G$ となるので、 $D_p\cong F\cong G$ 、すなわち $G$ と $D_p$ は同型である。
