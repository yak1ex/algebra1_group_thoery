...
5. $G$ が位数 $10$ の元を持てば、 $G$ には位数 $10$ の元が少なくとも $12$ 個あることを示し矛盾を導け ※正しくなさそう
位数 $10$ の元 $m$ があるとき、その元による生成群 $M$ を考えると $M=\langle m\rangle\cong\mathbb{Z}/10\mathbb{Z}\cong\mathbb{Z}/2\mathbb{Z}\times\mathbb{Z}/5\mathbb{Z}$ (最後は定理2.9.3中国式剰余定理より)。なので、 $M$ は位数 $5$ の部分群、すなわち $G$ のシロー $5$ 部分群を含む。これを $K$ 、$\mathbb{Z}/2\mathbb{Z}$ と同型側を $J$ とする。このとき $M=JK$ である。 $N_G(M)$ の定義( $\lbrace g\in G\mid gMg^{-1}=M\rbrace$ )の条件を書き換えると $gMg^{-1}=gJKg^{-1}=gJg^{-1}gKg^{-1}=JK=M$ となるから $|N_G(M)|\le|N_G(K)|$ である。 (3)より $|G|/|N_G(K)|=6$ である。 $M$ と共役な部分群の数は命題4.5.6より $|G|/|N_G(M)|\ge|G|/|N_G(K)|=6$ となるので少なくとも $6$ 個ある。 $M$ と共役な部分群それぞれについて少なくとも生成元とその逆元は位数 $10$ の元であり、しかも部分群ごとに異なる(さもなくば巡回群なので部分群全体が一致する)。よって、 $G$ が位数 $10$ の元を持つなら位数 $10$ の元は少なくとも $6\times2=12$ 個存在する。
